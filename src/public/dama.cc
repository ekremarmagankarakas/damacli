...
Result ToResult(GameResult r) {
  switch (r) {
    case GameResult::kWhiteWins: return Result::WhiteWins;
    case GameResult::kBlackWins: return Result::BlackWins;
    case GameResult::kDraw:      return Result::Draw;
  }
  return Result::Draw;
}
...
const char* ResultText(GameResult r) {
  switch (r) {
    case GameResult::kWhiteWins: return "White wins";
    case GameResult::kBlackWins: return "Black wins";
    case GameResult::kDraw:      return "Draw";
  }
  return "";
}
...
std::string ParseErrorText(ParseError e, std::string_view raw) {
  switch (e) {
    case ParseError::kEmpty:     return "Empty input";
    case ParseError::kBadSyntax: return "Bad input: " + std::string(raw);
    case ParseError::kBadSquare: return "Unknown square: " + std::string(raw);
    case ParseError::kIllegal:   return "Illegal move: " + std::string(raw);
  }
  return "";
}

std::unique_ptr<View> MakeView(ViewMode mode) {
  if (mode == ViewMode::Ascii) return std::make_unique<TextView>();
...
CommandResult Session::Apply(std::string_view cmd) {
  ...
  std::visit(
      Overloaded{
          [&](const Move& m) {
            // Parser already matched against LegalMoves(); IsLegal is defensive.
...
Result Session::Outcome() const {
  if (!impl_->is_over) return Result::Ongoing;
  if (!impl_->result) return Result::Draw;
